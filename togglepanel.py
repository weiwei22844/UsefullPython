...
class TestPanel(wx.Panel):
    ...
    def MakePaneContent(self, pane):
        ...
        self.name = wx.TextCtrl(pane, -1, "");
        self.name.SetMaxLength(1)
        self.Bind(wx.EVT_TEXT_MAXLEN, self.ResetContent, self.name)

        addrLbl = wx.StaticText(pane, -1, "Address:")
        self.addr1 = wx.TextCtrl(pane, -1, "");
        self.addr1.SetMaxLength(1)
        self.Bind(wx.EVT_TEXT_MAXLEN, self.ResetContent, self.addr1)
...
if __name__ == '__main__':
    app = wx.PySimpleApp()
    mainfrm = queryDlg(None, "QueryDlg")
    mainfrm.SetSize((800,600))
    mainfrm.Show()
    app.SetTopWindow(mainfrm)
    app.MainLoop()
